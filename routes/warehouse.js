...
router.get(["/", "/:id"], async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(successResponse(200, result.length === 1 ? result[0] : result));
  } catch (error) {
    logger.error(error);
...
router.post("/update", async (req, res) => {
  try {
    ...
    const query =
      "UPDATE TBL_WAREHOUSE SET NAME = ?, SKU = ? , ADDR_WR = ? WHERE id = ?";
    ...
    res
      .status(201)
      .json(successResponse(201, `Success Update Warehouse with id ${wrId}`));
  } catch (e) {
    logger.error(e);
  }
});
